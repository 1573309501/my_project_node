function add_add(_info_uid){
	...
	if(bool==true){
	 ...
	 console.log(_info_name);
	var _info_count=info_count.value;
	console.log(_info_count);
	...
	var formdata=`info_uid=${_info_uid}&info_name=${_info_name}&info_price=${_info_price}&info_count=${_info_count}&info_money=${_info_money}&info_date=${_info_date}&info_remark=${_info_remark}&info_type=${_info_type}`;
	xhr.send(formdata);
}
function add(_info_uid){
	//获取地址栏中查询字符串的uid
	//获取查询字符串地址栏对象
	...
	cz.innerHTML=
		`<table border="1" cellspacing="" cellpadding="" class="border-light border-0 w-100 my-1">
			<tr align="center"><th colspan="2" class="py-1">请录入消费信息</th></tr>
			<tr>
			...
			<tr>
				<td colspan="2" align="center">
					<button type="button" onclick="add_add(${_info_uid})" class="my-1 add_style btn " >添加</button>
					<input type="reset"  class="my-1 add_style btn" />
				</td>
			</tr>
		</table>`;
}